...
fn roundtrip_unnormalized_fixtures() {
    for entry in glob("./tests/fixtures/*.ftl").expect("Failed to read glob pattern") {
        ...
        let formatted_content = fs::read_to_string(formatted_path.join("junked").join(path.file_name().unwrap())).unwrap();
        assert_eq!(formatted_content, reserialized);
        let formatted = parse(formatted_content.as_str()).unwrap_or_else(|(res, _)| res);

        let formatted_content_without_junk = fs::read_to_string(formatted_path.join(path.file_name().unwrap())).unwrap();
        ...
        let reparsed_without_junk = parse(reserialized_without_junk.as_str()).unwrap_or_else(|(res, _)| res);
