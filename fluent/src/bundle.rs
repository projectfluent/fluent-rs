...
pub struct FluentBundle<'bundle> {
    ...
    pub resource_manager: Cow<'bundle, ResourceManager<'bundle>>,
}

impl<'bundle> FluentBundle<'bundle> {
    pub fn new<'a, S: ToString>(locales: &'a [S], res_mgr: Option<&'bundle ResourceManager<'bundle>>) -> FluentBundle<'bundle> {
        ...
        let res_mgr = match res_mgr {
            Some(res_mgr) => Cow::Borrowed(res_mgr),
            None => Cow::Owned(ResourceManager::new())
        };
        FluentBundle {
            locales,
            entries: HashMap::new(),
    ...
    pub fn add_messages(&'bundle mut self, source: &str) -> Result<(), Vec<FluentError>> {
        let res_mgr = &self.resource_manager;
        let res = res_mgr.get_resource_for_string(source);
        let mut errors = vec![];
